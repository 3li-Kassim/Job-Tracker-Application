...
export function ModalComp(props){
    ...
    const sendForm = async() => {
        if(
        formData.company === '' || formData.role === '' || formData.location === '' || 
        formData.status === '' || formData.dateApplied ==='')
        {
            setIsSuccess(false);
    ...
    return(
        <div className="modal show mt-5" style={{display: 'block', position:'fixed'}}>
            <Modal show={props.open} onHide={props.onClose} centered>
                ...
                <Modal.Body>
                    {displayMsg && <h4 className={`alert ${alert} text-center`}>{displayMsg}</h4>}
                    <form onSubmit={(e) => {e.preventDefault(); sendForm(); }}>
                        ...
                        <div >
                            <Button type="submit" className="signing-btn w-100 ">Submit</Button>
                        </div>
                    </form>
